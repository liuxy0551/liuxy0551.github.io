...
&ensp;&ensp;&ensp;&ensp;`jsencrypt`在处理一些较短文本的加密时比较好使，如 get 请求参数的加密。
``` shell
npm i jsencrypt -S
```
``` javascript
...
```
&ensp;&ensp;&ensp;&ensp;如果遇到 post 且参数较多时，就会报错`jsencrypt.js Message too long for RSA`，这个时候就需要`encryptlong`登场了。
