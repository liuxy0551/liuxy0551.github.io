...
&ensp;&ensp;&ensp;&ensp;`前端项目优化之旅`系列随笔主要记录工作中遇到的一些优化方案，这里记录一下引入和使用 CDN 的过程。
<!--more-->


### 一、为什么

&ensp;&ensp;&ensp;&ensp;当引入的组件越来越多时，运用 webpack 打包后的 app.js 文件容易变得过大，对首屏加载很不友好。
...
&ensp;&ensp;&ensp;&ensp;在`package.json`中删除即将通过 CDN 引入的组件，主要在 dependencies 和 devDependencies 中，删除 node_modules 后重新安装

#### 2、vue.config.js

&ensp;&ensp;&ensp;&ensp;引入 CDN 文件并配置 webpack，可以有效缓解加载速度慢的问题。
...
module.exports = {
  ...
  chainWebpack: config => {
    const cdn = {
      ...
      js: [
        process.env.NODE_ENV === 'development' ? 'http://wise-job.oss-cn-zhangjiakou.aliyuncs.com/webjs/libs/vue/v2.6.11.js' : '//wise-job.oss-cn-zhangjiakou.aliyuncs.com/webjs/libs/vue/v2.6.11.min.js',
...
&ensp;&ensp;&ensp;&ensp;在`<head></head>`标签中添加以下代码使用：
...
&ensp;&ensp;&ensp;&ensp;main.js 中可能有引入相关组件的 css 文件，当使用了 css 文件的 CDN 时，无需在 main.js 中继续引用。如：
