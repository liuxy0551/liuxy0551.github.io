---
title: 前端学习 Docker 之旅（二）—— 常用指令
urlname: docker-b
tags:
  - Docker
categories:
  - Docker
author: liuxy0551
copyright: true
date: 2020-02-05 11:07:16
updated: 2020-02-05 11:07:16
---


　　这两年 Docker 比较火，很多大厂都在使用。这里记录一下常用的指令。
<!--more-->


#### 1、查看本地镜像列表

``` shell
docker images
```
#### 2、docker run

　　docker 进程运行于独立的容器中，而容器是宿主机的一个进程，宿主机可以是本地的也可以是远程的。每当执行`docker run`之后，容器进程是独立运行的，它有自己的文件系统、网络以及与宿主机隔离的进程。[原文](https://docs.docker.com/engine/reference/run/#general-form)

#### 3、开始/停止某个容器

``` shell
docker start/stop [container_id]/[container_name]
```

#### 4、docker run 和 docker start 的区别

- docker run
docker run 只在第一次运行时使用，将镜像放到容器中，以后再次启动这个容器时，只需要使用命令 docker start 即可。docker run 相当于执行了两步操作：将镜像放入容器中（docker create），然后将容器启动，使之变成运行时容器（docker start）

- docker start
docker start 的作用是重新启动已存在的镜像。也就是说，如果使用这个命令，我们必须事先知道这个容器的 container_id，或者这个容器的名字，我们可以使用 docker ps 找到这个容器的信息

#### 5、列出正在运行的容器

``` shell
docker ps 或 docker container ls
```

#### 6、进入容器

``` shell
docker exec -it [container_id]/[container_name] /bin/bash
```

#### 7、列出所有容器

``` shell
docker ps -a 或 docker container ls -a
```

#### 8、删除某个镜像

``` shell
docker rmi -f [image_id]/[image_name]
```

#### 9、删除某个容器

``` shell
docker rm -f [container_id]/[container_name]
```

#### 10、重命名容器

``` shell
docker rename old_name new_name
```

#### 11、停止所有容器

``` shell
docker-compose stop
```
