...
&ensp;&ensp;&ensp;&ensp;git submodules 是为了多个项目（模块）都能够及时更新到最新的公共资源，解决需要把最新代码拷贝到各个项目的痛点。刚好最近公司开发的产品需要分为各个版本，但是主体功能一致，所以点上 git submodules 的技能点。对于 vue 项目，公共库可以分为 pages、components、styles 等几个目录。
<!--more-->


###  一、准备项目

&ensp;&ensp;&ensp;&ensp;**这篇随笔暂时只记录多个项目使用一个公共库的过程。**
&ensp;&ensp;&ensp;&ensp;准备好公共库（以 git-public 为例，称为子模块），准备好使用子模块的项目若干（以 git-a 和 git-b 为例，称为主项目）。
...
&ensp;&ensp;&ensp;&ensp;在 git-a 主项目下执行：`git submodules add <url> <path>`
``` shell
...
##### 2.2.1、clone
&ensp;&ensp;&ensp;&ensp;如果你是第一次在项目中引入子模块的开发人员，那么项目新加入的成员可以按照以下方法 clone 带有 submodules 的项目：
``` shell
...
##### 2.2.2、pull
&ensp;&ensp;&ensp;&ensp;如果你是第一次在项目中引入子模块的开发人员，那么项目的其他成员可以按照以下方法 pull 到带有 submodules 的项目：
``` shell
...
#### 3.1、获取子模块代码
&ensp;&ensp;&ensp;&ensp;在主项目中的子模块路径下执行：
``` shell
git pull origin master
```
&ensp;&ensp;&ensp;&ensp;多个子模块可在主项目路径下执行：
``` shell
...
&ensp;&ensp;&ensp;&ensp;删除公共库的步骤较为复杂，如下：
