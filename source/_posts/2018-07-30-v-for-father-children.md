...
　　![](http://media.liuxianyu.cn/v-for-father-children-jdfw.gif)
<!--more-->
...
　　![](http://media.liuxianyu.cn/v-for-father-children-1.png)

　　UI 理解：每个 Tab 点击后展现的页面都是这样的管理表格，所以这一部分表格写成组件。一开始采用的写法是下面这样的：

　　![](http://media.liuxianyu.cn/v-for-father-children-2.png)
...
　　![](http://media.liuxianyu.cn/v-for-father-children-3.png)

　　![](http://media.liuxianyu.cn/v-for-father-children-4.png)
　　<center>上图，定义的 tabList。</center>

　　因为要显示相应的数量，从接口中拿数据后与 tabList 进行拼接，不过给各位的参考意义不大，处理如下：
　　![](http://media.liuxianyu.cn/v-for-father-children-5.png)

　　下图将体现解决方法的核心，那就是带上对应的数组下标，即何时调用：
　　![](http://media.liuxianyu.cn/v-for-father-children-6.png)
...
　　![](http://media.liuxianyu.cn/v-for-father-children-7.png)

　　可以看出已经成了一个数组，这时候就需要我们加上下标再去调用相应的 tab 子组件方法，如下图，确定 tabList 的 index：
　　![](http://media.liuxianyu.cn/v-for-father-children-8.png)
